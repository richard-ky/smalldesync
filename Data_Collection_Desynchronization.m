...
%Desynchronization Data
 ...
 %%% Parameters to Vary
    AmpV = [10,30,50,70,100]'; %10 to 100 %10, 30, 50, 70, 100
    ...
    PW3V = [10,15,20,25,30,35,40,45,50]'; %ms between 10 to 50 (100 Hz to 20 Hz) %10,15,20,25
    NsV = (5:20:205)'; %5 to 205 pulses 
    J_IV = [450,400,350,300,250,200,150,100,50,40,30,20]'; % NOTE: J_I = J_IE 450 - 20 with interval of 50
    
    aE = -1;
    aI = 1;
    N_E = 80;% # of excitatory neurons
    N_I = 20;% # of inhibitory neurons
...
% -- Stimulation Input --
    ...
    plaston_param = 1; % 0 Plasticity OFF, 1: Plasticity ON
    counter = 1;
                ...
                for ll = 1:size(PW2V,1)
                    ...
                    for mm = 1:size(PW1V,1)
                        ...
                        for nn = 1:size(AmpV,1)
                            ...
                            [spike_E,spike_I,time,RE,spike_time_E,W_IE] = main_2000_neuron_network_EE_II(duration,step,duration_step,S_key_IE,S_key_EI,S_key_EE,S_key_II,J_E,J_EE,J_I,J_II,cross_100,Ue,Ui,stimon,stopEIWeight,sample_duration,stimon_param,plaston_param,W_IE0,W_EE0,W_EI0,W_II0); 
    
    
                            %Data Recorded - Firing Rates, Order Parameter, Spike timing   

                            %Calculating mean order parameter for 50ms
                            %before the start of stimulation and 50ms before the end of stimulation
	
                            idx = find(Ue ~= 0);
                            id1 = idx(1)-500:idx(1)-1;
                            nstep = PW3/step;
                            id2 = idx(end)-500:idx(end)-1;
                            Re1 = mean(RE(id1,1));
                            Re2 = mean(RE(id2,1));
